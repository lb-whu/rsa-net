...
class Generator:
  ...
  def __call__(self, input1,input2,segy,segz):
    """
    Args:
    ...
    with tf.variable_scope(self.name):
      ...
      c7s1_32_2 = ops.c7s1_k(input2, self.ngf, is_train=self.is_train ,is_training=self.is_training, norm=self.norm,
          reuse=self.reuse, name='c7s1_32_2')                             # (?, w, h, 64)
      d64_2 = ops.dk(c7s1_32_2, 2*self.ngf, is_train=self.is_train ,is_training=self.is_training, norm=self.norm,
          reuse=self.reuse, name='d64_2')                                 # (?, w/2, h/2, 128)
      d128_2 = ops.dk(d64_2, 4*self.ngf, is_train=self.is_train ,is_training=self.is_training, norm=self.norm,
          reuse=self.reuse, name='d128_2')                                # (?, w/4, h/4, 256)  
...
#      d128 = tf.concat( [d128_1, d128_2],3)    
      ...
      res_output = ops.n_res_blocks(d128_1,d128_2,segy = segy,segz = segz, is_train=self.is_train ,reuse=self.reuse, n=9)      # (?, w/4, h/4, 128)
...
#          reuse=self.reuse, name='ad128')
      ...
      u64 = ops.uk(res_output, 2*self.ngf,is_train=self.is_train , is_training=self.is_training, norm=self.norm,
