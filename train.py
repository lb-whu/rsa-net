      ...
      while not coord.should_stop():
        ...
        for i in range(len(file_paths_X)):
          if not coord.should_stop():
            ...
            x_val1 = aug.zm(x_val)
            x_val2 = aug.zm(x_val1)

            if step < 50000:#train unet
              ...
              zx_val1 = aug.zm(zx_val)
              zx_val2 = aug.zm(zx_val1)
#              fakey_val = sess.run(fake_y,feed_dict={cycle_gan.y: y_val,cycle_gan.z:z_val})
              _, Dice_val,summary = (
                  sess.run([optimizersU, DiceTrain,summary_op],feed_dict={ad_gan.x: x_val,ad_gan.y: y_val,ad_gan.segy: x_val2,ad_gan.segz:zx_val2}))

            elif (step >= 50000):#train GAN
              if flag == 1:
                if swit == 1:
                  swit = 0
                ...
                zx_val1 = aug.zm(zx_val)
                zx_val2 = aug.zm(zx_val1)
                fakey_val = sess.run(fake_y,feed_dict={ad_gan.y: y_val,ad_gan.z:z_val,ad_gan.segy: x_val2,ad_gan.segz:zx_val2})
                fakey_val1 = aug.zm(fakey_val)
                fakey_val2 = aug.zm(fakey_val1)
                
                
                _, G_z_val, D_z_val,Dice_val,diceSIM_val,summary = (
                    sess.run([optimizers, G_gan_loss_z, D_Z_loss,DiceLoss,summary_op],
                          feed_dict={ad_gan.x: x_val,ad_gan.y: y_val,ad_gan.z:z_val,ad_gan.segy: x_val2,ad_gan.segz:zx_val2,
                ...
                if step % 50 == 0:
                  ...
                  if is_finished:
                    for n in range(len(val_files)):
                      ...
                      fakey_val = sess.run(fake_y,feed_dict={ad_gan.y: y_val,ad_gan.z:z_val,ad_gan.segy: x_val2,ad_gan.segz:zx_val2})
                      fakey_val = np.squeeze(fakey_val)
                      sio.savemat(os.path.join('/result/',val_file.split('/')[-1]),{'img':fakey_val,'x':x_val,'y':y_val,'z':z_val},do_compression = True) 
                    flag = 0
                    swit = 1
              else:
                if swit == 1:
